bl_info = {
    ...
    "version": (1, 13),
    "blender": (3, 0, 0),
    "location": "Shader Editor > Sidebar > CompactMAT",
    "description": "Advanced material creation and assign compact panel",
    "warning": "",
    "doc_url": "",
    "category": "Material",
}
...
def connect_texture(mat, img, target_socket_name, context_type="COLOR", normal_mode='NORMAL'):
    ...
    op_rm = bpy.ops.compactmat.remove_texture(socket_name=target_socket_name, context_type=context_type)
...
class COMPACTMAT_OT_pick_material(bpy.types.Operator):
    ...
    bl_idname = "compactmat.pick_material"
...
class COMPACTMAT_OT_new_material(bpy.types.Operator):
    bl_idname = "compactmat.new_material"
...
class COMPACTMAT_OT_remove_material(bpy.types.Operator):
    bl_idname = "compactmat.remove_material"
...
class COMPACTMAT_OT_assign_material(bpy.types.Operator):
    bl_idname = "compactmat.assign_material"
    bl_label = "Assign"
    bl_options = {'REGISTER', 'UNDO'}
...
class COMPACTMAT_OT_setup_displacement(bpy.types.Operator):
    ...
    bl_idname = "compactmat.setup_displacement"
...
class COMPACTMAT_OT_upload_all_textures(bpy.types.Operator, ImportHelper):
    bl_idname = "compactmat.upload_all_textures"
...
class COMPACTMAT_OT_add_texture(bpy.types.Operator, ImportHelper):
    bl_idname = "compactmat.add_texture"
...
class COMPACTMAT_OT_remove_texture(bpy.types.Operator):
    bl_idname = "compactmat.remove_texture"
...
def draw_socket_row(layout, socket, context_type="COLOR", show_prop=True, custom_name=None):
    ...
    if not socket.is_linked:
        op = row.operator("compactmat.add_texture", text="", icon='FILE_FOLDER')
        ...
        if tex_node and tex_node.image:
            ...
            op_add = row.operator("compactmat.add_texture", text="", icon='FILE_FOLDER')
            op_add.socket_name = socket.name
            op_add.context_type = context_type
            
            op_rm = row.operator("compactmat.remove_texture", text="", icon='X')
...
class COMPACTMAT_PT_main_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.operator("compactmat.pick_material", text="", icon='EYEDROPPER')
        ...
        row = layout.row()
        row.operator("compactmat.new_material", text="New", icon='ADD', depress=True)
        
        row = layout.row(align=True)
        row.operator("compactmat.assign_material", text="Assign", icon='MATERIAL', depress=True)
        row.operator("compactmat.remove_material", text="Unassign", icon='TRASH', depress=True)
        ...
        layout.operator("compactmat.upload_all_textures", text="Upload All Textures", icon='NODETREE')
        layout.separator()
        ...
        def draw_disp(c):
            ...
            split.operator("compactmat.setup_displacement", text="Setup Displacement", icon='MOD_SUBSURF')
